-- E11 — Envío inmediato: el cliente puede pedir que salga en el próximo
-- turno disponible en vez de esperar el lead_dias normal, pagando un extra.
-- Regla simple: si pide antes de `corte_inmediato_hora` (hora del negocio),
-- el turno más cercano es la tarde de hoy; si pide después, es la mañana de
-- mañana. envio-form.js calcula ese turno y, si el día que le toca está
-- cerrado (domingo, feriado, corredor lleno), sigue probando el día
-- siguiente — para eso necesita poder preguntarle a cupos_disponibles() por
-- una fecha sin que la rechace por "sin_anticipacion" (que es justo la
-- regla que este envío paga por saltarse).
--
-- El monto real que se cobra NO sale del cliente: pedidos_sellar() sigue
-- siendo la última palabra (mismo criterio que ya usa para costo_envio) y
-- suma costo_envio_inmediato server-side cuando ve envio_inmediato = true.

alter table public.envio_config
  add column if not exists costo_envio_inmediato numeric not null default 0 check (costo_envio_inmediato >= 0),
  add column if not exists corte_inmediato_hora time not null default '13:00';

alter table public.pedidos
  add column if not exists envio_inmediato boolean not null default false;

-- ── cupos_disponibles: mismo cálculo, con un escape para "sin_anticipacion" ─
drop function if exists public.cupos_disponibles(date, date, uuid, uuid);

create or replace function public.cupos_disponibles(p_desde date, p_hasta date, p_zona_id uuid, p_sucursal_id uuid default null, p_ignorar_anticipacion boolean default false)
returns table (
  fecha date,
  disponible boolean,
  motivo text,
...
begin
  ...
  left join lateral (
    select case
      ...
      when not p_ignorar_anticipacion and dd.f < public.hoy_ar() + v_lead then 'sin_anticipacion'
...
revoke execute on function public.cupos_disponibles(date, date, uuid, uuid, boolean) from public;
grant execute on function public.cupos_disponibles(date, date, uuid, uuid, boolean) to anon, authenticated;

-- ── pedidos_sellar: bypass de sin_anticipacion + recargo, del lado del servidor ─
...
begin
  ...
  new.envio_inmediato := coalesce(new.envio_inmediato, false);
  ...
  begin
    ...
    if found and new.zona_id is not null then
      ...
      if new.envio_inmediato then
        new.costo_envio := coalesce(new.costo_envio, 0) + coalesce(v_cfg.costo_envio_inmediato, 0);
      end if;
    end if;
    ...
    if new.fecha_entrega is not null then
      ...
      elsif not new.envio_inmediato and new.fecha_entrega < public.hoy_ar() + v_lead then
        new.necesita_revision := true;
